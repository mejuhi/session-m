...
class SplitCustomer1(beam.DoFn):
    def process(self, element):
        ...
        # print("hello")
        arr = bytes(email)
        encoded_text = cipher_suite.encrypt(arr)

        return [{
            'myid': str(myid),
            'name': str(name),
            'lastname': str(lastname),
            'email': str(encoded_text),
...
class ConvertToJsonDict(beam.DoFn):
    def process(self, data_item):
        ...
        json2_data["phone_numbers"] = [{"phone_number": str(json1_data["attr2"]), "phone_type": "mobile"}]
        json2_data["external_id"] = "   "
        json2_data["opted_in"] = "true"
        json2_data["external_id_type"] = "  "
        json2_data["locale"] = "    "
        json2_data["ip"] = "    "
        json2_data["dob"] = "   "
        json2_data["address"] = "   "
