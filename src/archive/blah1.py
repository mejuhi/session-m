...
class save_me2(beam.DoFn):
    def process(self, data_item):
        ...
        #f.writerow(["external_id", "opted_in", "external_id_type", "email", "locale", "ip", "dob", "address", "city", "state", "zip", "country", "gender", "first_name", "last_name", "referral", "phone_numbers", "custom_1", "custom_2"])
        #print(x["external_id"])
        #print(type(x["external_id"]))
        #for x in x:
...
stage1 = str(result)
print("STAGE1: Validating mail id of users: " + stage1)
...
result = p.run().wait_until_finish()
stage2 = str(result)
print("STAGE2: Joining the two tables: " + stage2)
...
result = p.run().wait_until_finish()
stage3 = str(result)
print("STAGE3: Store json for table 1: " + stage3)
...
result = p.run().wait_until_finish()
stage4 = str(result)
print("STAGE4: Store json for table 2: " + stage4)
...
json1_csv = (p
             ...
             | 'Save json 1 ' >> beam.ParDo(save_me1())
             )

json2_csv = (p
             | 'Read json 2' >> ReadFromText(file_pattern='../output-data2/SendToAPI*')
             | 'Save json 2 ' >> beam.ParDo(save_me2())
             )

result = p.run().wait_until_finish()
stage5 = str(result)
print("STAGE5: Store json for table 2: " + stage4)
